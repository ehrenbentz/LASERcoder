@echo off
REM build_LASERcoder_Windows.bat
REM Run from inside LASERcoder\build_Windows\
REM
REM Directory structure:
REM   LASERcoder\
REM     current_version.txt    Shared version file
REM     src\              Python source files including main.py
REM     icons\            App logo ICO/PNG files
REM     libs\mpv\win64\   libmpv-2.dll
REM     build_Windows\    This script, LASERcoder.iss
REM       dist_Windows_x64_v#_#_#\ Created by this script (arch + version-stamped)
REM         LASERcoder_v{ver}_windows_x64_setup.exe    Installer
...
set APP_NAME=LASERcoder
set MAIN_SCRIPT=main.py
set SOURCE_DIR=..\src
set VERSION_FILE=..\current_version.txt
set LIBMPV_FILE=..\libs\mpv\win64\libmpv-2.dll
set ICON_FILE=..\icons\laser.ico

REM Code signing — reads cert and password from files in user home directory.
REM Falls back to LASERCODER_CERT_FILE / LASERCODER_CERT_PASS env vars if files not found.
...
if exist "%HOME_CERT%" (
    ...
    if defined LASERCODER_CERT_FILE set CERT_FILE=%LASERCODER_CERT_FILE%
    if defined LASERCODER_CERT_PASS set CERT_PASS=%LASERCODER_CERT_PASS%
)
...
if not exist "%SOURCE_DIR%" (
    ...
    echo Run this script from inside LASERcoder\build_Windows\
    exit /b 1
)
...
if not exist "%LIBMPV_FILE%" (
    echo ERROR: libmpv-2.dll not found at %LIBMPV_FILE%
    exit /b 1
)

if not exist "%ICON_FILE%" (
    echo ERROR: laser.ico not found at %ICON_FILE%
    exit /b 1
)
...
copy "%ICON_FILE%" "%OUTPUT_DIR%\laser.ico" >nul
copy "%LIBMPV_FILE%" "%OUTPUT_DIR%\libmpv-2.dll" >nul
...
python -m nuitka ^
    ...
    --windows-product-name=LASERcoder ^
    --windows-file-description="LASERcoder - Lightweight Annotation Software for Ethology Research" ^
    %MAIN_SCRIPT%
...
REM =====================================================================
REM Rename dist folder (Nuitka names it after the source file, not the exe)
REM =====================================================================
...
if exist %ISCC% (
    if exist "LASERcoder.iss" (
        if defined CERT_FILE if defined CERT_PASS (
            %ISCC% /DSignEnabled "/Smysign=signtool.exe sign /f !CERT_FILE! /p !CERT_PASS! /fd sha256 /td sha256 /tr http://timestamp.digicert.com $f" /DAppVer=%APP_VERSION% "LASERcoder.iss"
        ) else (
            %ISCC% /DAppVer=%APP_VERSION% "LASERcoder.iss"
        )
        if !errorlevel! neq 0 (
        ...
        echo WARNING: LASERcoder.iss not found. Skipping installer creation.
    )
) else (
