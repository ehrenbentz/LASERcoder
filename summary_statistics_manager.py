...
class SummaryStatisticsManager(QDialog):
    ...
    def __init__(self, parent=None, initial_dir=str(Path.home())):
        ...
        dialog_width = int(self.display_width * 0.7)
    ...
    def create_file_section(self):
        ...
        self.file_listbox.setSelectionMode(QAbstractItemView.SelectionMode.NoSelection)  # We'll use checkboxes instead
    ...
    def ensure_summary_dir(self, base_dir):
        """Ensure that the Summary_Statistics directory exists."""
        summary_dir = os.path.join(base_dir, "Summary_Statistics")
        try:
            os.makedirs(summary_dir, exist_ok=True)
            return summary_dir
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Could not create Summary_Statistics directory: {str(e)}")
            return None
    
    def generate_individual_summaries(self):
        """Generate individual summary statistics for selected files without progress dialogs."""
        ...
        for file_path in selected_files:
            try:
                ...
                output_file = generate_summary_statistics(file_path)
                if output_file:
                    success_count += 1
            except Exception as e:
    ...
    def generate_combined_summary(self):
        """Generate a combined summary for all selected files without progress dialogs."""
        selected_files = self.get_selected_files()
        ...
        try:
            # 1. Combine all annotations into one file
            combined_annotations = self.combine_annotation_files(selected_files, experiment_name, base_dir)
            
            if combined_annotations:
                # 2. Skip running summary statistics on the combined file
                # We'll only generate the meta-summary from individual files
                
                # 3. Generate individual summaries if they don't exist yet
                individual_summaries = []
                
                for file_path in selected_files:
                    try:
                        video_name = os.path.basename(file_path).replace("_Annotations.csv", "")
                        summary_path = os.path.join(summary_dir, f"{video_name}_Summary.csv")
                        
                        if not os.path.exists(summary_path):
                            # Generate the summary if it doesn't exist
                            summary_path = generate_summary_statistics(file_path)
                        
                        if summary_path and os.path.exists(summary_path):
                            individual_summaries.append(summary_path)
                    except Exception as e:
                        print(f"Error processing {file_path}: {e}")
                
                # 4. Combine the individual summaries
                if individual_summaries:
                    meta_summary_path = os.path.join(summary_dir, f"{experiment_name}_Meta_Summary.csv")
                    combine_summaries(individual_summaries, meta_summary_path)
                    
                    # Success message
                    QMessageBox.information(
                        self, 
                        "Combined Analysis Complete", 
                        f"Successfully created analysis for '{experiment_name}'.\n\n"
                        f"The following files were created:\n\n"
                        f"Combined annotations file:\n{os.path.basename(combined_annotations)}\n\n"
                        f"Meta-Summary:\n{experiment_name}_Meta_Summary.csv\n\n"
                        f"Files saved to:\n{summary_dir}"
                    )
                else:
                    QMessageBox.warning(
                        self,
                        "Warning",
                        "No valid summary files could be created from the selected annotations."
                    )
            else:
                QMessageBox.critical(
                    self, 
                    "Error", 
                    "Failed to create combined annotations file."
                )
        except Exception as e:
            QMessageBox.critical(
                self, 
    ...
    def combine_annotation_files(self, file_paths, experiment_name, base_dir):
        """
        ...
        Args:
            ...
            base_dir: Base directory where output files should be saved
        ...
        combined_file_path = os.path.join(annotations_dir, f"{experiment_name}_Annotations_Combined.csv")
